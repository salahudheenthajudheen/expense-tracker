...
export const TransactionForm = ({ onSubmit }: TransactionFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3.5 rounded-2xl bg-et-card p-4 shadow-sm ring-1 ring-slate-100 sm:space-y-4 sm:p-5">
      <div className="flex items-center gap-2.5 sm:gap-3">
        <div className={`rounded-xl p-2.5 sm:rounded-2xl sm:p-3 ${isIncome ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
          <Plus className="h-4 w-4 sm:h-5 sm:w-5" />
        </div>
        <div>
          <h3 className="text-base font-semibold text-slate-900 sm:text-lg">Add transaction</h3>
          <p className="hidden text-sm text-slate-500 sm:block">Track income or expenses instantly.</p>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
        <label className="flex flex-col gap-1.5 sm:gap-2">
          <span className="text-xs font-medium text-slate-600 sm:text-sm">Amount (₹)</span>
          <input
            type="number"
            inputMode="decimal"
            min={0}
            ...
            className="rounded-xl border border-slate-200 px-3 py-2.5 text-base font-semibold text-slate-900 outline-none focus:border-indigo-500 sm:px-4 sm:py-3 sm:text-lg"
            placeholder="1,500"
          />
        </label>

        <label className="flex flex-col gap-1.5 sm:gap-2">
          <span className="text-xs font-medium text-slate-600 sm:text-sm">Date</span>
          <input
            type="date"
            value={formValues.date}
            onChange={(event) => setFormValues((prev) => ({ ...prev, date: event.target.value }))}
            className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-indigo-500 sm:px-4 sm:py-3 sm:text-base"
            max={todayDate()}
          />
        </label>
      </div>
      ...
      {!isIncome && (
        <div>
          <p className="text-xs font-medium text-slate-600 sm:text-sm">Category</p>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-xs font-medium transition sm:gap-2 sm:px-3 sm:text-sm ${
                    isSelected ? styles : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >
      ...
      <label className="flex flex-col gap-1.5 sm:gap-2">
        <span className="text-xs font-medium text-slate-600 sm:text-sm">Note (optional)</span>
        <div className="flex items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 focus-within:border-indigo-500 sm:rounded-2xl sm:px-4 sm:py-3">
          <Tag className="mt-0.5 h-3.5 w-3.5 text-slate-400 sm:mt-1 sm:h-4 sm:w-4" />
          <textarea
            rows={2}
            value={formValues.description}
            onChange={(event) => setFormValues((prev) => ({ ...prev, description: event.target.value }))}
            placeholder={isIncome ? 'Salary, freelance, etc.' : 'Fuel top-up, movie ticket, groceries, etc.'}
            className="w-full resize-none bg-transparent text-xs text-slate-900 outline-none sm:text-sm"
          />
        </div>
      </label>
